...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            result_row.exists = bool(check.data.get("data", {}).get("exists"))
            if not result_row.exists:
                result_row.save()
                self.stdout.write(f"check_http={check.http_status} exists=false send=skipped")
                return
            ...
            result_row.save()
            self.stdout.write(f"check_http={check.http_status} send_http={sent.http_status} success={str(result_row.success).lower()} message_id={result_row.provider_message_id or '-'} state={result_row.provider_state or '-'}")
